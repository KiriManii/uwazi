  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes',
        ...
        () => {
          loadPoll(); // Refresh results when new votes come in
        }
      )
      .subscribe();
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-4 mb-8">
        {poll.options
          ...
          .map((option, index) => {
            ...
            return (
              <div key={option.id} className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
                <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-medium text-lg">{option.option_text}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl font-bold text-gray-900">{percentage}%</span>
                    <p className="text-sm text-gray-500">{option.vote_count} votes</p>
                  </div>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ease-out ${
                      index === 0 ? 'bg-gradient-to-r from-primary-600 to-primary-700' : 
                      index === 1 ? 'bg-gradient-to-r from-secondary-600 to-secondary-700' :
      ...
      {poll.total_votes === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-600 mb-4">Be the first to vote on this poll!</p>
          <button
